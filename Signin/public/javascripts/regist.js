$().ready(function() {
    ...
    $(inputs[2]).click(function() {
        ...
        $(inputs[2]).blur(function() {
            checkRePassword($(inputs[1]).val(), $(inputs[2]).val());
        });
    });
    $(inputs[3]).click(function() {
        ...
        $(inputs[3]).blur(function() {
            checkId($(inputs[3]).val());
        });
    });
    $(inputs[4]).click(function() {
        $(checks[4]).removeClass("right");
        $(checks[4]).removeClass("false");
        $(inputs[4]).blur(function() {
            checkTelephone($(inputs[4]).val());
        });
    });
    $(inputs[5]).click(function() {
        $(checks[5]).removeClass("right");
        $(checks[5]).removeClass("false");
        $(inputs[5]).blur(function() {
            checkMail($(inputs[5]).val());
        });
    });
    if (hint.html() !== "hint") {
        hint.addClass("show");
...
function checkPassword(password) {
    if (password !== "" && password.match(/^[\w]{6,12}$/) != null) {
...
function checkTelephone(tele) {
    if (tele !== "" && tele.match(/^[1-9]{1}[0-9]{10}$/) != null) {
        $(".check").eq(4).addClass("right");
    } else {
        $(".check").eq(4).addClass("false");
    }
}

function checkMail(mailbox) {
    if (mailbox !== "" && mailbox.match(/[\w!#$%&'*+\/=?^_`{|}~-]+(?:\.[\w!#$%&'*+\/=?^_`{|}~-]+)*@(?:[\w](?:[\w-]*[\w])?\.)+[\w](?:[\w-]*[\w])?/) != null) {
        $(".check").eq(5).addClass("right");
    } else {
        $(".check").eq(5).addClass("false");
    }
}

function check() {
    var _right = $(".right");
    if (_right.length !== 6) return false;
    else return true;
}